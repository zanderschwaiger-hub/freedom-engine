    ...
    <div style={{ background: 'var(--navy)', color: 'var(--white)', minHeight: '100vh' }}>
      ...
      <section style={{ padding: `clamp(56px, 9vw, 110px) ${pad} clamp(28px, 4vw, 48px)`, maxWidth: '820px', margin: '0 auto' }}>
        ...
        <p style={{ fontSize: '17px', fontWeight: 300, color: 'var(--dim)', lineHeight: 1.7 }}>
          A workforce-readiness program that trains your people, assesses what they know, and produces documented
          proof of preparedness — aligned with recognized business-continuity and organizational-resilience principles.
        </p>
      </section>
      ...
      <section style={{ padding: `0 ${pad} clamp(32px, 5vw, 56px)`, maxWidth: '820px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          <div style={card}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              ...
              <li style={li}><span style={dash}>—</span><span>Finance, HR, operations, and client-facing staff — the most-targeted roles</span></li>
          ...
          <div style={card}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
              ...
              <li style={li}><span style={dash}>—</span><span>You receive a per-employee certificate + a company evidence report</span></li>
        ...
        {!showForm && (
          <div style={{ textAlign: 'center', marginTop: 'clamp(36px, 5vw, 56px)' }}>
            <button className="btn-primary" onClick={openForm} style={{ fontSize: '16px', padding: '18px 40px' }}>
              Request the program →
            </button>
          ...
          <div style={{ ...card, boxShadow: '0 8px 40px rgba(15,23,42,0.05)' }}>
            {status === 'done' ? (
              ...
              <>
                ...
                <button className="btn-primary" onClick={submit} disabled={status === 'sending'} style={{ fontSize: '15px', padding: '16px 32px', width: '100%', opacity: status === 'sending' ? 0.6 : 1 }}>
                  {status === 'sending' ? 'Sending…' : 'Request business program info'}
                </button>
              </>
            )}
          </div>
          <p style={{ fontSize: '11px', fontWeight: 300, color: 'rgba(15,23,42,0.45)', lineHeight: 1.6, marginTop: '28px', textAlign: 'center' }}>
            Freedom Engine is a private, independently reviewable program. This credential confirms completion of the Freedom Engine assessment. It is not ISO certification, insurer approval, legal-compliance certification, or a guarantee of insurance coverage. Insurance eligibility and coverage are determined solely by the applicable insurer.
          </p>
        </section>
      )}
    </div>
